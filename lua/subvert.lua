...
local function camelcase(word)
  word = word:gsub('%-', '_')
  if not word:find '_' and word:match '%l' then return (word:gsub('^.', string.lower)) end
  local out = ''
  local first = true
...
local function uppercase(word) return snakecase(word):upper() end

local function dashcase(word) return (snakecase(word):gsub('_', '-')) end

local function spacecase(word) return (snakecase(word):gsub('_', ' ')) end

local function dotcase(word) return (snakecase(word):gsub('_', '.')) end
...
local function expand_braces(dict)
  ...
  if redo then return expand_braces(new_dict) end
  return new_dict
end
...
local function subesc(s) return (s:gsub('([%]%[/\\%.%*%+%?~%%%(%)&])', '\\%1')) end
...
local function build_pattern(dict, boundaries)
  ...
  return '\\v\\C' .. a .. '%(' .. table.concat(parts, '|') .. ')' .. b
end
...
local function parse_subvert_args(bang, count, raw_args)
  local args = raw_args
  if args:match '^[%w]' or args == '' then args = (bang and '!' or '') .. args end
  ...
  elseif #parts == 2 and parts[2]:match '^[A-Za-z]*n[A-Za-z]*$' then
    return {
      mode = 'substitute',
      parts = { parts[1], '', parts[2] },
      sep = sep_char,
    }
  elseif #parts == 2 and parts[2]:match '^[A-Za-z]*[%+%-]?%d*$' then
    return {
      mode = 'search',
      pattern = parts[1],
      flags = parts[2],
...
local function build_substitute_cmd(range, bad, good, flags_input)
  ...
  return range .. 's/' .. pat .. "/\\=v:lua.require('subvert')._abolished()/" .. sub_flags, dict, pat
end
...
local function do_grep(bang, pattern, flags_str, grep_args)
  ...
  vim.cmd('grep' .. (bang and '!' or '') .. ' ' .. lhs .. ' ' .. grep_args)
end
...
local function err_echo(s) vim.api.nvim_echo({ { s .. '\n', 'ErrorMsg' } }, true, {}) end
...
local function execute_subvert(bang, line1, line2, count, raw_args)
  ...
  if #parts < 2 then
    err_echo 'Subvert: E471: Argument required'
    return
  end
  if #parts > 3 then
    err_echo 'Subvert: E488: Trailing characters'
    return
  end
...
local function preview_subvert(opts, preview_ns, preview_buf)
  ...
  local ok_parse, parsed = pcall(parse_subvert_args, bang, count, raw_args)
  if not ok_parse or parsed.mode ~= 'substitute' then return 0 end
  ...
  for lnum = start_line, end_line do
    ...
    if #matches > 0 then
      ...
      if preview_buf and preview_buf >= 0 then preview_lines[#preview_lines + 1] = string.format('|%d| %s', lnum, new_line) end
    end
  end

  if not has_matches then return 0 end
...
function M.setup(user_opts)
  ...
  vim.api.nvim_create_user_command('Subvert', function(o) execute_subvert(o.bang, o.line1, o.line2, o.range, o.args) end, cmd_opts)

  local has_S = false
  pcall(function()
    local info = vim.api.nvim_parse_cmd('S test', {})
    if info and info.cmd and info.cmd ~= 'Subvert' then has_S = true end
  end)
  if not has_S then vim.api.nvim_create_user_command('S', function(o) execute_subvert(o.bang, o.line1, o.line2, o.range, o.args) end, cmd_opts) end
end
